...
def test_presets_qwen_y_gemini_tienen_roles_y_contexto_grande():
    presets = {
        item["id"]: item
        for item in ai.listar_presets_ia()
    }

    qwen = presets["openrouter-qwen3-coder-free"]
    gemini = presets["gemini-2.5-flash-free"]

    assert qwen["model_id"] == "qwen/qwen3-coder:free"
    assert qwen["role"] == "coder_primary"
    assert qwen["context_window"] >= 1_000_000

    assert gemini["model_id"] == "gemini-2.5-flash"
    assert gemini["role"] == "analyst_secondary"
    assert gemini["base_url"].endswith("/v1beta/openai")


def test_lab_coder_usa_presupuesto_dinamico_y_no_pide_8192_por_defecto():
    provider = ai.AIProviderConfig(
        provider_id="llmlab",
        provider_name="Laboratorio UTB",
        model_id="lab-coder",
    ...
    # Aproxima un prompt grande similar al que antes provocaba
    # 12289 input + 8192 output > 20480.
    system_prompt = "s" * 2000
    user_content = "x" * 33000
    max_tokens = ai._dynamic_output_tokens(
        provider,
...
def test_contexto_se_compacta_para_proveedor_pequeno():
    provider = ai.AIProviderConfig(
        provider_id="llmlab",
        provider_name="Laboratorio UTB",
        model_id="lab-coder",
    ...
    payload = {
        "codigo_relevante_redactado": "A" * 70000,
    ...
    compacted, changed = ai._compact_payload_for_provider(
        provider,
    ...
    assert len(serialized) < len(json.dumps(payload))


def test_routing_prefiere_qwen_y_tras_fallo_gemini(
    tmp_path,
    monkeypatch,
):
    config_path = tmp_path / "ai-provider.json"
    ...
    initial = ai.resolver_cadena_proveedores_ia(
        task="patch",
    )
    after_failure = ai.resolver_cadena_proveedores_ia(
        task="diagnosis",
        has_failures=True,
    )

    assert initial[0].role == "coder_primary"
    assert initial[0].model_id == "qwen/qwen3-coder:free"
    assert after_failure[0].role == "analyst_secondary"
    assert after_failure[0].model_id == "gemini-2.5-flash"
    assert any(item.role == "fallback" for item in initial)


def test_fallback_cambia_de_qwen_a_gemini_si_principal_falla(
    tmp_path,
    monkeypatch,
):
    config_path = tmp_path / "ai-provider.json"
    ...
    def fake_post(url, headers, json, timeout):
        ...
        if json["model"] == "qwen/qwen3-coder:free":
    ...
    data, provider, trace = ai._solicitar_json_con_fallback(
        None,
    ...
    assert calls[0][1] == "qwen/qwen3-coder:free"
